...
def backward(img1, img2, M):
    ...
    '''
    h, w, c = img1.shape
    h_2, w_2, c_2 = img2.shape

    sigma = 0.5
    mask_size = 5

    result = np.zeros((h_2, w_2, c_2))
    img_window = np.zeros((mask_size,mask_size,c_2))

    for row in range(h_2):
        for col in range(w_2):
            ...
            for i in range(-2,3) :
                for j in range(-2, 3):
                    if x_+j >=0 and y_+i>=0 and x_+j<=w-1 and y_+i<=h-1 :
                        # print(x_+j, y_+i)
                        img_window[i+2 , j+2,:] = my_bilinear(img1, x_ + j, y_ + i)
                        gaussian_mask = my_get_Gaussian2D_mask(5,sigma)
                        # gaussian_weight = np.exp((-1 / 16) * ((i+2) ** 2 + (j+2) ** 2))

            img_window[:, :, 0] = img_window[:, :, 0] * gaussian_mask
            img_window[:, :, 1] = img_window[:, :, 1] * gaussian_mask
            img_window[:, :, 2] = img_window[:, :, 2] * gaussian_mask

            val = np.sum(gaussian_mask[:, :, 0])
            if val > 255 :
                val = 255
            result[row, col, 0] = val

            val = np.sum(gaussian_mask[:, :, 1])
            if val > 255 :
                val = 255
            result[row, col, 1] = val

            val = np.sum(gaussian_mask[:, :, 2])
            if val > 255 :
                val = 255
            result[row, col, 2] = val
    return result
...
def my_get_Gaussian2D_mask(msize, sigma):
    ...
    for i in range(-msize//2+1, msize//2+1):
        for j in range(-msize//2+1, msize//2+1):
            temp[i, j] = exp(temp[i, j]) * 1/(2 * np.pi)
...
def feature_matching_RANSAC(img1, img2, keypoint_num=None, iter_num=500, threshold_distance=10):
    ...
    for i in range(iter_num):
        ...
        for idx, match in enumerate(matches) :
            ...
            if L2_distance(xy_prime[0:2], (kp2_x, kp2_y)) < threshold_distance :
                cnt += 1
        inliers.append((cnt))
...
def main():
    ...
    src2 = cv2.imread('../image/LenaFaceShear.png')


    result_RANSAC = feature_matching_RANSAC(src, src2)
    result_LS = feature_matching(src, src2)
    cv2.imshow('input', src)
    cv2.imshow('result Backward_Warping 201902739', result_RANSAC)
    # cv2.imshow('result LS 201902739', result_LS)

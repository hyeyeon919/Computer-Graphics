...
def feature_matching_RANSAC(img1, img2, keypoint_num=None, iter_num=500, threshold_distance=10):
    ...
    for i in range(iter_num):
        ...
        ls_M = my_ls(three_points, kp1, kp2)

        # 2. 1에서 뽑은 matches를 이용하여 affine matrix M을 구함
        affine_M = np.array([[ls_M[0][0], ls_M[1][0], ls_M[2][0]],
                      [ls_M[3][0], ls_M[4][0], ls_M[5][0]],
                      [0, 0, 1]])
        M_list.append(affine_M)

        # 3. 2에서 구한 M을 모든 matches point와 연산하여 inlier의 개수를 파악
        cnt = 0
        for point in matches :
            M = np.dot(affine_M, np.array([[kp1[point.queryIdx].pt[0], kp1[point.queryIdx].pt[1], 1]]).T)
            v_1= np.array([M[0][0], M[1][0]])
            v_2 = np.array(kp2[point.trainIdx].pt)

            if L2_distance(v_1, v_2) < threshold_distance :
                cnt += 1
...
def main():
    src = cv2.imread('../image/library1.jpg')
    src2 = cv2.imread('../image/library2.jpg')

    result_RANSAC = feature_matching_RANSAC(src, src2)
    result_LS = feature_matching(src, src2)
    cv2.imshow('input', src)
    cv2.imshow('result RANSAC 201902739', result_RANSAC)

...
def get_integral_image(src): #integral 이미지로 만든다 크기는 src와 동일
    ##########################################################################
    # ToDo
...
def calc_local_integral_value(src, left_top, right_bottom): #integral이미지를 입력받아 4개의 값을 연산해 요소합을 구한다.
...
def harris_detector(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            # R 계산 Harris 방정식 구현
            ##########################################################################
            det_M = np.linalg.det(M_harris[row][col])
            trace_M = np.trace(M_harris[row][col])
...
def harris_detector_integral(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            # R 계산 Harris 방정식 구현
            ##########################################################################
            det_M = np.linalg.det(M_integral[row][col])
            trace_M = np.trace(M_integral[row][col])
            R[row, col] = det_M - k * (trace_M * trace_M)
...
def main():
    ...
    cv2.imshow('harris_img ' + '201902739' , harris_img)
    cv2.imshow('harris_integral_img ' + '201902739' , harris_integral_img)
    cv2.waitKey()

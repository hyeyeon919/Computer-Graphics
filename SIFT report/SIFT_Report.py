...
def SIFT(src):
    ...
    for i in range(len(keypoints)):
        ...
        ################################zzzzz###################################
        for j in range(0,36) :
            if orient_hist.max() * 0.8 < orient_hist[j] :
                keypoints.append(cv2.KeyPoint(x, y, size, j * 10, response, octave, class_id))
        keypoints[i].angle = orient_hist.argmax() * 10.0
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                if(descriptor_angle > 360) :
                    descriptor_angle -= 360
                index = (int)((descriptor_angle) // 45)
                new_row, new_col = make_new(row, col)
                arr[new_row,new_col,index] += magnitude[p_y, p_x] * gaussian_weight
            cnt = 0
            for a in range (0,4) :
                for b in range(0,4) :
                    descriptors[i][cnt * 8 : (cnt+1) * 8] = arr[a,b]
                    cnt = cnt+1
...
def main():
    ...
    # 결과의 학번 작성하기!
    cv2.imshow('my_sift 201902739', result)
    cv2.waitKey()

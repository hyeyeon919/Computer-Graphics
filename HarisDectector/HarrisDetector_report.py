...
def calc_derivatives(src):
    ...
    (h, w) = src.shape
    src_pad = my_padding(src, sobel_x)
    (fx_h, fx_w) = sobel_x.shape
    Ix = np.zeros((h, w), dtype=float)

    for row in range(h):
        for col in range(w):
            sum = np.sum(src_pad[row:row + fx_h, col:col + fx_w] * sobel_x)
            Ix[row, col] = sum

    src_pad = my_padding(src, sobel_y)
    (fy_h, fy_w) = sobel_y.shape
    Iy = np.zeros((h, w), dtype=float)

    for row in range(h):
        for col in range(w):
            sum = np.sum(src_pad[row:row + fy_h, col:col + fy_w] * sobel_y)
            Iy[row, col] = sum
...
def HarrisDetector(src, gaus_filter_size = 3, gaus_sigma = 1, alpha = 0.04, threshold_rate = 0.01):
    ...
    # cv2.imshow('201902739 dst_x_Norm', dst_x_Norm)
    # cv2.imshow('201902739 dst_y_Norm', dst_y_Norm)
    # cv2.waitKey()
    # cv2.destroyWindow('201902739 dst_x_Norm')
    # cv2.destroyWindow('201902739 dst_y_Norm')
    ...
    # cv2.imshow('201902739 IxIx', IxIx)
    # cv2.imshow('201902739 IyIy', IyIy)
    # #cv2.imshow('IxIy', IxIy)
    # cv2.imshow('201902739 dst_IxIy_Norm', dst_IxIy_Norm)
    # cv2.waitKey()
    # cv2.destroyWindow('201902739 IxIx')
    # cv2.destroyWindow('201902739 IyIy')
    # #cv2.destroyWindow('IxIy')
    # cv2.destroyWindow('201902739 dst_IxIy_Norm')
...
#def GaussianFiltering(src, fshape = (3,3), sigma=1):
    ...
    # cv2.imshow('201902739 G_IxIx', G_IxIx)
    # cv2.imshow('201902739 G_IyIy', G_IyIy)
    # #cv2.imshow('G_IxIy', G_IxIy)
    # cv2.imshow('201902739 G_dst_IxIy_Norm', G_dst_IxIy_Norm)
    # cv2.waitKey()
    # cv2.destroyWindow('201902739 G_IxIx')
    # cv2.destroyWindow('201902739 G_IyIy')
    # #cv2.destroyWindow('G_IxIy')
    # cv2.destroyWindow('201902739 G_dst_IxIy_Norm')
    ...
    """
    har = (G_IxIx * G_IyIy) - (G_IxIy**2) - 0.04 * ((G_IxIx + G_IyIy)**2)

    #0 ~ 1 사이의 값으로 변경 후 0 ~ 255로 변경 -> 결과가 잘 나왔는지 확인하기위해서
    G_dst_har_Norm = ((har - np.min(har) )/np.max(har - np.min(har)) * 255 + 0.5).astype(np.uint8)
    cv2.imshow('201902739 har before threshold', G_dst_har_Norm)
    ...
    cv2.imshow('201902739 har after threshold', G_dst_har_thresh_Norm)
...
def main():
    ...
    cv2.imshow('201902739 original', src)
    ...
    cv2.imshow('201902739 interest points', interest_points)
    cv2.imshow('201902739 harris_img', harris_img)
    cv2.waitKey()
